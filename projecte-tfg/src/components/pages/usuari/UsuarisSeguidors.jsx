...
export const UsuarisSeguidors = ({id, usuarisSeguint, setUsuarisSeguint}) => {
  ...
  const conseguirUsuaris = async (pg) => {
    try{
        ...
        const peticio = await fetch(Global.url+"follow/followers/"+idUsuari+"/"+pg, opcions);
        const dades = await peticio.json();
        if (dades.status != "error"){  
            setContador(dades.total);
            if(pg == 1){
              let follows = dades.follows.map(follow => follow.user);
              setLlistat(follows);
            }
            else{
              let follows = dades.follows.map(follow => follow.user);
  ...
  const cerca = async (e) => {
    ...
    if(e.target.nick.value != ""){
        try{
            ...
            let idUsuari = auth._id;
            if(id && id != 1)idUsuari = id;
            const peticio = await fetch(Global.url+"follow/cerca/seguidors/"+idUsuari+"?nick="+e.target.nick.value, opcions);
            const dades = await peticio.json();
            if (dades.status != "error"){
              let follows = dades.usuaris.map(follow => follow.user);
...
const segPagina = () => {
  ...
  return (
    <div className='tabInformacioLlistatsUsuaris'>
        <h2 className='aligncenter'>El segueixen {contador} usuaris</h2>   
        <LlistatPersones
            cerca={cerca}
